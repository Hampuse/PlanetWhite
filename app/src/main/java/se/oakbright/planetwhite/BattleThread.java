...
public class BattleThread extends Thread implements SurfaceObserver {
	
	private static final String TAG = BattleThread.class.getSimpleName();
	
	//fps
	private final static int 	MAX_FPS = 50;	// desired fps
	private final static int	MAX_FRAME_SKIPS = 5;	
	private final static int	FRAME_PERIOD = 1000 / MAX_FPS;	//ms? the frame period
	/* Stuff for stats */
	...
	private BattleModel battleModel;	// The actual view that handles inputs and draws to the surface 
	private final BattleSurface[] battleSurfaces;
	...
	public BattleThread(BattleModel battleModel, BattleSurface[] battleSurfaces) {
		super();
		//this.surfaceHolder = surfaceHolder;
		this.battleModel = battleModel;
		this.battleSurfaces = battleSurfaces;
		for(int i = 0; i < battleSurfaces.length; i++){
			battleSurfaces[i].registerSurfaceObserver(this);
		}
	}
	...
	public void run() {
		...
		Log.d("thread", "in BattleThread run()");
		//this.gameModel.setup();
		...
		while (running) {
			for(int i = 0; i < battleSurfaces.length; i++){
				canvas = null;	
				surfaceHolder = battleSurfaces[i].getHolder(); 
				try {											// try locking the canvas for exclusive pixel editing in the surface
					...
					synchronized (surfaceHolder) {
						...
						battleModel.update(); //TODO INTE S utanf!
						battleSurfaces[i].update(); 	
						battleSurfaces[i].render(canvas);		// render state to the screen, draws the canvas on the panel
						...
						while (sleepTime < 0 && framesSkipped < MAX_FRAME_SKIPS) {
							...
							this.battleModel.update();  	// we need to catch up. update without rendering
...
//			Log.d(TAG, "Average FPS:" + df.format(averageFps));
	...
	public void notifySurfaceCreated(BattleSurface surface) {
		...
		for(int i = 0; i<battleSurfaces.length; i++){
			if( this.battleSurfaces[i] == null)
				allCreated = false;
		}
		if(allCreated == true)
			this.setRunning(true);
			this.start();*/ //is set running by gameActivity instead
	}
	...
	public static final int getMicroSecondsPerUpdate(){
		return FRAME_PERIOD*1000;
	}
}